export interface IUnemployment {
  ...
  females_unemployment_rate: number;
  males_unemployment_rate: number;
  no_studies: number;
  primar: number;
  gimnazial: number;
  highschool: number;
  post_highschool: number;
  professional: number;
  universitar: number;
  under25: number;
  between25_29: number;
  ...
  urban: number;
  rural: number;
  females_urban: number;
  females_rural: number;
  males_urban: number;
  males_rural: number;
  county_name: string;
}

export function getFieldValue(object: IUnemployment, field: string): number {
  switch (field) {
    case "total":
      return object.total;
    case "females":
